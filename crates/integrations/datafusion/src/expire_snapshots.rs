...
use iceberg::transaction::{
    ApplyTransactionAction, Transaction, UnreferencedFiles, unreferenced_files,
};
...
/// [`unreferenced_files`]: it selects the snapshots to expire, commits the metadata change through
/// a [`Transaction`], and then deletes the now-unreferenced files. The selection knobs forward to
/// the core action; see it for their exact semantics.
///
...
impl ExpireSnapshots {
    ...
    pub async fn execute(self, catalog: &dyn Catalog) -> Result<ExpireSnapshotsResult> {
        ...
        Ok(ExpireSnapshotsResult::new(expired_snapshot_ids, &files))
    }
}
...
/// Outcome of an [`ExpireSnapshots`] run: the expired snapshots and the per-kind counts of files
/// deleted.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ExpireSnapshotsResult {
    /// Snapshot ids that were expired, sorted ascending.
...
impl ExpireSnapshotsResult {
    fn new(expired_snapshot_ids: Vec<i64>, files: &UnreferencedFiles) -> Self {
        Self {
