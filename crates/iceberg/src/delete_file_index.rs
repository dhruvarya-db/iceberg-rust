...
// pauses just before creating its `Notified` future, giving the populating task time
...
impl DeleteFileIndex {
    ...
    pub(crate) async fn get_deletes_for_data_file(
        ...
        // Test seam: see `repro_lost_wakeup_when_sender_dropped_before_query`. The `Notified`
        // future below is created only after this point, so a `notify_waiters()` fired during
        // this pause is lost and the await never completes.
        #[cfg(test)]
...
mod tests {
    ...
    //
    // This test FAILS (hangs -> times out) on this unfixed code. The companion PR that creates
    // the `Notified` future while holding the read lock makes it pass.
